...
(Figure~\ref{fig:window}). After dropping windows with too little coverage, this
yields 1{,}657 meal excursions from 45 subjects.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig-window.png}
\caption{An example postprandial excursion, the mean meal response of one
...
split, 80 percent into a gallery and 20 percent held out. A nearest-neighbor
...
of $1/45$ and $5/45$. We repeat this over 20 random gallery and probe splits and
report the mean.
...
the leaders: peak and baseline glucose keep adjusted ICCs of 0.24 (95\% CI 0.15
to 0.33) and 0.20 (0.10 to 0.30). In
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig1-distance.png}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig2-importance.png}
...
this directly, we re-ran the classifier on feature subsets
(Table~\ref{tab:ablation}). Removing peak and baseline glucose lowers the AUC from
0.87 to 0.71, and dropping every amplitude feature, leaving only timing, kinetic,
and shape, still reaches 0.66, well above the 0.5 chance level. Glucose level
alone gives 0.85. Amplitude therefore carries most of the signal, as the label
makes inevitable, but the shape of the excursion predicts diabetes on its own.

\begin{table}[!t]
\centering
\caption{Diabetes prediction under feature ablation (subject-grouped CV).}
\label{tab:ablation}
\begin{tabular}{lc}
\hline
Feature set & AUC \\
...
result is expected almost by definition. The ablation in Table~\ref{tab:ablation}
speaks to this: with the amplitude features removed, the shape and kinetic
features still predict above chance, so the result is not only a restatement of
glucose level. The re-identification accuracy is well above chance but
...
The CGMacros dataset is publicly available from PhysioNet. All feature-extraction
and analysis code is open source at
\href{https://github.com/sunjungco2027-hub/cgm-individuality}{\nolinkurl{github.com/sunjungco2027-hub/cgm-individuality}}, and a single command
regenerates every number reported here. The pipeline is deterministic: random
splits are seeded and the re-identification result is averaged over twenty fixed
splits. A test suite pins the reported values so that any change moving them is
caught, and a separate set of tests checks the intraclass correlation against an
independent one-way ANOVA computation and verifies the effect-size and
permutation calculations on synthetic data with known answers. The results in
this paper correspond to the tagged release \texttt{v1.0} of the repository.

\section*{Acknowledgment}
The author thanks Dr. Seo Ho Song for guidance and feedback throughout this
project. This work uses the CGMacros dataset, and the author thanks its creators
and PhysioNet for making the data openly available.
